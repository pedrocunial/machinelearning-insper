...
tinyMceWrapper.Editor.prototype =
{
    ...
    handleWindowClick : function ( event )
    {
      ...
      if ( typeof(className) !== "undefined" && className !== null && className.indexOf( "mce" ) == -1 )
      {
        ...
        {
          // On the 'submit' event remove Spell Check Markup before finalizing editor content.
          if ( event.target.type == "submit" )
          {
            var tinyMceEditor = tinyMceWrapper.editors.get( tinyMceWrapper.currentEditorId )._tinyMceEditor;
            if ( tinyMceEditor )
            {
              tinyMceEditor.execCommand('clearSpellCheckMarkUp');
            }
          }
